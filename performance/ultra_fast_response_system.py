...
class UltraFastResponseSystem:
    ...
    async def generate_ultra_fast_response(
        ...
        try:
            ...
            # ENHANCE: Add memory context to the message for better retrieval
            enhanced_message = message
            if enhanced_memory_system:
                try:
                    # Get memory context
                    memory_context = enhanced_memory_system.get_cached_context()
                    if memory_context:
                        # Import the enhanced formatting function
                        try:
                            from enhance_memory_retrieval import enhance_memory_formatting
                            enhance_memory_formatting()
                            from dynamic_character_playground_enhanced import _format_memory_context_for_agent
                            context_summary = _format_memory_context_for_agent(memory_context)
                            if context_summary:
                                enhanced_message = f"{message}\n\n🎯 MEMORY CONTEXT:\n{context_summary}"
                                print(f"🚀 Enhanced ultra-fast response with memory context")
                        except ImportError:
                            # Fallback to basic context
                            if memory_context.get('recent_memories'):
                                recent_memories = memory_context['recent_memories'][:3]
                                context_text = "\n".join([f"- {m.get('content', '')}" for m in recent_memories])
                                enhanced_message = f"{message}\n\nRecent memories:\n{context_text}"
                except Exception as e:
                    print(f"Memory context enhancement failed: {e}")
                    enhanced_message = message
            
            # Generate response with enhanced message
            response = agent.run(enhanced_message, user_id=user_id)
