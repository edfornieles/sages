...
class MoodSystem:
    ...
    def update_mood(self, user_message: str, memory_db_path: str = None) -> Dict:
        """Update character mood based on user interaction"""
    ...
    def _shift_mood_category(self, current_category: str, current_level: int, change_amount: int) -> Tuple[str, int]:
        """Shift mood to a different category based on major changes"""
        if change_amount >= 2:  # Very positive interaction
            if current_category in ["sad", "angry", "anxious"]:
            ...
            # Don't force excited/euphoric characters to become angry immediately
            # Instead, make them transition more gradually
            if current_category == "excited" and current_level >= 2:  # thrilled or euphoric
                # Excited characters become anxious first, not angry
                return "anxious", min(2, current_level)
            elif current_category == "happy" and current_level >= 2:  # joyful or ecstatic
                # Very happy characters become sad first, not angry
                return "sad", 1
            elif current_category == "playful" and current_level >= 1:
                ...
                # Other characters become angry but at a lower level
                return "angry", min(2, max(1, current_level))
